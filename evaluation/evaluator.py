...
class Eval_thread():
    ...
    def run(self):
        ...
        if self.logfile != "":
            self.LOG(
                '  {:.4f} {:.4f} {:.4f} {:.4f}  '.format(s,max_f,max_e, mae))
        return '[cost:{:.4f}s]{} dataset with {} method get {:.4f} {:.4f} {:.4f} {:.4f} ..'.format(
            time.time() - start_time, self.dataset, self.method, s,max_f,max_e, mae)
        '''
        self.LOG(
